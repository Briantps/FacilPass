...
@Stateless(mappedName = "ejb/LowBalance")
public class LowBalanceEJB implements LowBalance{
	...
	public ArrayList<ReAlarmBalance> getAcountUser (long UserId) {
		System.out.println("getAcountUser EJB" + UserId );
		ArrayList<ReAlarmBalance> list = new ArrayList<ReAlarmBalance> ();
		try {
			
			Query q= em.createNativeQuery("SELECT DISTINCT vw.account_id cuenta, nvl(tab.val_minimo,0) SaldoMinimo, tab.val_min_alarm SaldoBajo,tb.bank_aval Aval,(CASE "+ 
											  "WHEN tb.bank_aval = 1 "+ 
											  "THEN (select 'El Saldo Bajo no prodrá ser inferior a $'|| dbms_lob.substr( system_parameter_value,20, 1) from tb_system_parameter where system_parameter_id = 4) "+
											  "WHEN tb.bank_aval = 0 "+
											...
											"WHERE usrs= ?1 "+
											"AND tab.id_tip_alarm=4 "+
											"ORDER BY vw.account_id DESC");
			...
			for(int i=0;i<lis.size();i++){
				...
				r.setMsn(o[4]!=null?o[4].toString():"-");
	...
	public Long getCountAcountUser (long UserId){
		Long cont = 0L;
		try {
		...
		try {
			...
			for (int i = 0; i < TemUpdatepaccountclient.size(); i++) {
				...
				if (value.getAccount()!= null&& value.getValbajo()!=null){
					...
					Query q = em.createNativeQuery("update tb_alarm_balance set val_min_alarm = ?2 where account_id = ?1 and id_tip_alarm = 4");
					...
					if (count > 0){
						 ...
						 */
						TbProcessTrack pt = process.searchProcess(ProcessTrackType.CLIENT, usrs);
						Long idPTA;
						if(pt==null){
							idPTA=process.createProcessTrack(ProcessTrackType.CLIENT,  usrs, ip, usrs);
						}
						else{
							idPTA=pt.getProcessTrackId();
						}
						Long detailA=process.createProcessDetail(idPTA,ProcessTrackDetailType.UPDATE_LOW_BALANCE, 
								"Se realizó " +
						...
						//proceso cliente
						TbProcessTrack ptc = process.searchProcess(ProcessTrackType.MY_CLIENT_PROCESS,usrs);
						Long idPTC;
						if(ptc==null){
							idPTC=process.createProcessTrack(ProcessTrackType.MY_CLIENT_PROCESS, usrs, ip, usrs);
						}
						else{
						...
						log.insertLog("Actualizacion de Saldo Bajo | Cuenta: " +formateador(new BigDecimal(value.getAccount()))+" Valor: " + valueList, 
								LogReference.USER,

...
public class ImageSelectorBean implements Serializable{
	...
	public List<SelectItem> getStationList() {
		...
		if(!conce.equals(def)){
			...
			if(lis.size()==0){
				...
				stationList.add(new SelectItem(-3, "Seleccione La Estación"));
	...
	public List<SelectItem> getDeviceList() {
		...
		//deviceList.add(new SelectItem(-3, "Seleccione Dispositivo"));
			...
			if(initDate.getTime()<=endDate.getTime()){
				...
				if(conce!=null && conce!=-1L){
					if(idClient!=null && idClient!=-1L){
						if(codClient!=null){
							if(!codClient.equals("")){
								if(Validation.isNumeric(codClient)){
								...
								this.setMessageModal("El Campo No. Identificación es requerido.");
								this.setShowModal(true);
							}
						...
						this.setMessageModal("No existe un cliente con el número de identificación digitado.");
						this.setShowModal(true);
					}
				}
				else{
					this.setMessageModal("El campo concesión es requerido.");
					this.setShowModal(true);
				}
	...
	public String getPictures(){
		...
		try {
			...
			String hour= dateT.substring(11,19);
			hour=hour.replace(":", "");
			hour=hour.trim();
...
//		Long tran= Long.parseLong(obj);
	...
	public String saveNotification(){
		try{
			...
			if(typeId==0L){
				this.setShowNotification(true);
	            this.setMessageNotification("Error: Debe Seleccionar Un Tipo De Problema o Indicar Que No Aplica");
			}
			else{
				if(idPhoto!=null){
					if(typeId>0L){
						notification="SE MARCÓ IMAGEN COMO PROBLEMA";
					}
					if(image.createNotification(idPhoto, date, name, notification, typeId,user, transactionId,idClient,dateTransaction)){
						this.setShowNotification(true);
			            this.setMessageNotification("Notificacion creada correctamente");
			            this.setShowModal(false);
					}else{
						this.setShowModal(true);
			            this.setMessageModal("Error: al crear notificación");
					}
				}
				else{
					this.setShowModal(true);
	...
	public String getNotifications(){
		try{
			System.out.println("Estoy en getNotifications()");
			System.out.println("id" + transactionId2);
			
			listNotifications=image.getNotifications(transactionId2);
			...
			if(listNotifications.size()>0){
				...
				this.setShowListNotifications(false);
				this.setMessageNotification("No se encontraron notificaciones para esta transacción");
				this.setShowNotification(true);
			}
		}catch(Exception ex){
			ex.printStackTrace();
	...
	public String search() {
		...
		accountList.clear();
		idClient = -1L;
		accountList.add(new SelectItem(-3,"Seleccione Cuenta"));
		this.setShowModal(false);
		this.setShowData(false);
		...
		if (!codClient.equals("")) {
			if(Validation.isNumeric(codClient)){
				...
				if (client != null) {
					...
					this.setShowModal(true);
					this.setMessageModal("No existe información para el cliente digitado.");
					this.setShowData(false);
				}
			}
			else{
	...
	public List<SelectItem> getTypes() {
		...
		types.add(new SelectItem(0, "SELECCIONE"));
		types.add(new SelectItem(-1, "NO APLICA"));
		for (TbCodeImagesRejected t : image.getCodesImagesRejected()){
    ...
    public String showNoti(){
    	...
    	if(typeId<=0){
    		...
    		this.setNotification(this.notification+ "  -  SE MARCARÁ IMAGEN COMO PROBLEMA");

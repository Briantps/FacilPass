...
public class ReportIndividualByVehicleBeanAdmin extends AbstractBaseReportBean implements Serializable{
	...
	private final String COMPILE_FILE_NAME="reportIndividualByVehicle";
	...
	public ReportIndividualByVehicleBeanAdmin(){
	...
	protected String getFileName() {
		return "ReportIndividualByVehicle" + System.currentTimeMillis();
	}
	...
	protected Map<String, Object> getReportParameters() {
		...
		if(accountId==-1){
			reportParameters.put("account", "TODAS LAS CUENTAS");
		}
		else{
			if(accountId!=null){
		...
		if(plateId==-1){
			reportParameters.put("placa", "TODAS LAS PLACAS");
		}
		else{
			if(plateId!=null){
			...
			if(codeClient!=""){
				if(accountId>0){
					if(plateId>0){
						if(dateIni!=null && dateEnd!=null){
							...
							if(dateIni.getTime() >= fechaIniParametrizada.getTime()){
								if(dateEnd.getTime()>= dateIni.getTime()){
								  ...
								  this.setShowMessageError(true);
								  this.setMessageError("El reporte Recibo Individual Por Placa se generará a partir de " + fechaInicial);
								  this.setShowData(false);
							}	
						...
						this.setShowMessageError(true);
						this.setMessageError("Error: Seleccione una placa para generar el reporte Recibo Individual por Placa.");
						this.setShowData(false);
					}
	...
	public List<SelectItem> getPlateList() {
		...
		if(this.getAccountId()!=null){
			if(this.getAccountId()<=0){
				plateList.add(new SelectItem("-1","Seleccione Placa"));	
			}
			else{

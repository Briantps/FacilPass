...
public class ReportTraceabilityVehicleBeanAdmin extends AbstractBaseReportBean implements Serializable{
	...
	private final String COMPILE_FILE_NAME="reportTraceabilityVehicle";
	...
	public ReportTraceabilityVehicleBeanAdmin(){
		accountId=0L;
		plateId=0L;
		...
		plateList.add(new SelectItem(0L, "Seleccione cuenta"));
	...
	protected String getFileName() {
		return "ReportTraceabilityVehicles" + System.currentTimeMillis();
	}
					...
					if(plateId!=0){
						if(dateIni!=null && dateEnd!=null){
							...
							if(dateIni.getTime()>=fechaBase.getTime()){
								...
								this.setShowMessageError(true);
								this.setMessageError("El reporte Trazabilidad Por Vehículo se generará a partir de "+ fechaI);
								this.setShowData(false);
							}
						}
						else{
		 ...
		 if(plateList!=null){
			 ...
			 plateList.add(new SelectItem("-1","TODAS LAS PLACAS"));
				if(accountId!=null && accountId>0){
					System.out.println("else accountId en getPlateList" + accountId );
					 plateList.clear();
					plateList.add(new SelectItem("-1","TODAS LAS PLACAS"));
					...
					System.out.println("else plateId en getPlateList" + plateId );
			}
		 }
					...
					if(lis!=null){
						if(lis.size()>0 && lis.size()==1){
							...
							if(plateList!=null){
								plateList.clear();
								if(accountId==-1L){
									System.out.println("if accountId en getPlateList" + accountId );
									plateList.add(new SelectItem("-1","TODAS LAS PLACAS"));	
									System.out.println("if plateId en getPlateList" + plateId );
								}

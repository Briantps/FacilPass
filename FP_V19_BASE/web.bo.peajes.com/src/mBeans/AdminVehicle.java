...
/**
 * Bean of vehicles for client
 * @author jromero
 */
public class AdminVehicle implements Serializable {
	...
	private boolean postValidationSearch(){
		if((categoryType == -1L || categoryType == 0L) && 
			(placaFil.equals("")) && 
			(ad1Fil.equals("")) && 
			(ad2Fil.equals("")) && 
			(ad3Fil.equals(""))) { 
			this.setMsg("No existe datos para realizar la búsqueda.");
			this.setShowModal(true);
			return false;
	...
	public boolean validateModifiedField(){
		...
		if(vehicleEJB.isEspecialPlate(vehiclePlate.toUpperCase())==true&&especialForm==false){
			this.setMsg("El vehículo tiene una placa especial," +
				" por favor seleccione la casilla Especial y escriba una observación.");
			return false;
		}
		if(vehicleEJB.isEspecialPlate(vehiclePlate.toUpperCase())==false&&especialForm==true){
			this.setMsg("El vehículo no tiene una placa especial, " +
				"por favor no seleccione la casilla Especial.");
			return false;
		}
		...
		if(validate){
			...
			if(validate){
				if(newCategoryId==0L){
					...
					if(vehicleInternalCode==null||vehicleInternalCode.equals("")){
						validate=true;
					}else{
					...
					if(validate){
						if(line==null||line.trim().equals("")){
							...
							if(!line.matches("([a-z]|[A-Z]|[0-9]|[/]|[(]|[)]|[_-]|[.]|[ñÑ]|\\s)+")){
								...
								if(line.length()>100){
									...
									if(aditional1==null||aditional1.equals("")){
										...
										if(!aditional1.matches("([a-z]|[A-Z]|[0-9]|[/]|[(]|[)]|[_-]|[.]|[ñÑ]|\\s)+")){
											validate=false;
									...
									if(validate){
										if(aditional2==null||aditional2.equals("")){
											...
											if(!aditional2.matches("([a-z]|[A-Z]|[0-9]|[/]|[(]|[)]|[_-]|[.]|[ñÑ]|\\s)+")){
												validate=false;
										...
										if(validate){
											if(aditional3==null||aditional3.equals("")){
												...
												if(!aditional3.matches("([a-z]|[A-Z]|[0-9]|[/]|[(]|[)]|[_-]|[.]|[ñÑ]|\\s)+")){
													validate=false;
	...
	public boolean validateField(){
		...
		if(vehiclePlate==null||vehiclePlate.trim().equals("")){
			...
			userId=userEJB.getSystemMasterById(usrs.getUserId());
			System.out.println("AdminVehicle.userId: "+userId);
			typeRelation=vehicleEJB.typeRelationTag(vehiclePlate.toUpperCase());
			System.out.println("validateField.typeRelation: "+typeRelation);
			userIdVehicle=vehicleEJB.getUserIdByVehicleId((vehicleEJB.getVehicleByPlate(vehiclePlate.
					toUpperCase())==null?-1L:vehicleEJB.getVehicleByPlate(vehiclePlate.toUpperCase()).
							getVehicleId()));
				...
				if(vehiclePlate.length()>12){
					...
					if(!vehiclePlate.matches("([a-z]|[A-Z]|[0-9])+")){
						...
						if((vehicleEJB.plateExit(vehiclePlate.toUpperCase()))&&
								((typeRelation==1L)||(typeRelation==0L)||((typeRelation==-1L)&&(userId.equals(userIdVehicle))))){
							this.setMsg("El vehículo "+vehiclePlate.toUpperCase()+" ya se encuentra registrado en el sistema.");
							return false;
						}else if(((vehicleEJB.plateExit(vehiclePlate.toUpperCase()))&&(typeRelation==-1L))||
								(vehicleEJB.plateExit(vehiclePlate.toUpperCase())==false)){
							if(vehicleEJB.isPlacaValida(vehiclePlate.toUpperCase())){
								...
								System.out.println("Pasó validación placa especial");
							}
						}
					}
				}
		...
		try{
			...
			if(vehicleEJB.plateExit(vehiclePlate.toUpperCase())){//
				...
				if(vehicleEJB.getLastInactiveRelationTag(vehicleEJB.getVehicleByPlate(vehiclePlate.toUpperCase())
						.getVehicleId())!=-1L){
				result=vehicleEJB.reasignVehicle(SessionUtil.sessionUser().getUserId(),
						SessionUtil.ip(), vehicleEJB.getVehicleByPlate(vehiclePlate.toUpperCase()).getVehicleId(),
						userId);
				}else{
					corr=false;
				}
				...
				result=vehicleEJB.createVehicle(SessionUtil.sessionUser()
						...
						vehiclePlate, vehicleChassis, vehicleInternalCode,
						newColorId, brandId,
						newCategoryId,
						aditional1, aditional2, aditional3, line,
						vehicleBeginingDate,observation, userId);
			}
			if(corr!=false){
			if(result==-1L){
				mensaje="El vehículo "+vehiclePlate.toUpperCase()+" ya se encuentra registrado en el sistema.";
				System.out.println("El vehículo con placa "+vehiclePlate.toUpperCase()+
				...
				if(exist){
					boolean result1=vehicleEJB.alterVehicle(SessionUtil.sessionUser()
							.getUserId(), SessionUtil.ip(),vehicleEJB.getVehicleByPlate(vehiclePlate.toUpperCase()).getVehicleId(),
							vehicleChassis, vehicleInternalCode,
							newColorId, brandId,
							newCategoryId,
							aditional1, aditional2, aditional3, line,
							vehicleBeginingDate,observation,especialForm, userId);
					System.out.println("Actualización información - AdminVehicle: "+result1);
				}
				...
				vehicleEJB.createProcessForVehicles(SessionUtil.sessionUser()
						.getUserId(), SessionUtil.ip(), messDetail, "Error Crear Vehículo", 
						userId, 1L);
	...
	public String deleteVehicle(){
		...
		if(resul){
			mensaje="Se eliminó correctamente el vehículo con placa "
				+vehiclePlate.toUpperCase()+".";
			//administrador
			String messDetail="Se eliminó correctamente el vehículo con placa "+vehiclePlate.toUpperCase()+".";
			
			vehicleEJB.createProcessForVehicles(SessionUtil.sessionUser()
					.getUserId(), SessionUtil.ip(), messDetail, "Error Eliminar Vehículo", 
					userId, 1L);
			
		}else{
			if(vehicleEJB.haveRelationTag(vehicleID)){
				mensaje="No se pudo eliminar el vehículo con placa "
					+vehiclePlate.toUpperCase()+", tiene tags registrados.";
			}else{
				mensaje="No se pudo eliminar el vehículo con placa "
					+vehiclePlate.toUpperCase()+".";
			}
	...
	public String showModalEdit(){
		...
		if(ve!=null){
			this.setAditional1(ve.getAditional1()==null?"":ve.getAditional1());
			this.setAditional2(ve.getAditional2()==null?"":ve.getAditional2());
			this.setAditional3(ve.getAditional3()==null?"":ve.getAditional3());
			this.setLine(ve.getLine()==null?"":ve.getLine());
			this.setNewBrand1(ve.getTbBrand()==null?"":ve.getTbBrand().getBrandName());
			this.setNewCategoryId(ve.getTbCategory().getCategoryId());
			this.setNewCatId(0L);
			this.setNewColorId(ve.getTbColor().getColorId());
			this.setBrandId(ve.getTbBrand()==null?null:ve.getTbBrand().getBrandId());
			this.setVehicleBeginingDate(ve.getVehicleBeginingDate());
			this.setVehicleChassis(ve.getVehicleChassis()==null?"":ve.getVehicleChassis());
			this.setVehicleInternalCode(ve.getVehicleInternalCode()==null?"":ve.getVehicleInternalCode());
			this.setVehiclePlate(ve.getVehiclePlate()==null?"":ve.getVehiclePlate());
			this.setObservation(ve.getVehicleObservation()==null?"":ve.getVehicleObservation());
			if(ve.getVehicleEspecialPlate()==null||ve.getVehicleEspecialPlate()==0L){
				this.setEspecialForm(false);
				this.setShowObservation(false);
	...
	public String storeEdited(){
		System.out.println("AdminVehicle.sotreEdited");
		...
		try{
			...
			result=vehicleEJB.alterVehicle(SessionUtil.sessionUser()
					...
					vehicleBeginingDate,observation,especialForm, userId);
			if(result){
				mensaje="El vehículo con la Placa "+vehiclePlate.toUpperCase()
				+" se ha modificado correctamente.";
				System.out.println("El vehículo con la Placa "+vehiclePlate.toUpperCase()
						+" se ha modificado correctamente.");
				
				String messDetail="Se modificó correctamente el vehículo con placa "+vehiclePlate+".";
				
				vehicleEJB.createProcessForVehicles(SessionUtil.sessionUser()
						.getUserId(), SessionUtil.ip(), messDetail, "Error Modificar Vehículo", 
						userId, 1L);
			}else{
				mensaje="Ocurrió un error al momento de modificar el vehículo.";
				System.out.println("Ocurrio un error al momento de modificar el vehículo.");
			}
			hideModalAll();
			clearFilter();
			System.out.println("the result:"+result);
	...
	public List<Vehicles> getListVehicles(){
		...
		if(search==false){
			...
			listVehicles=vehicleEJB.getUserVehiclesByFilters(userEJB.getCodeTypeId(userId).longValue(),
					userEJB.getSystemUser(userId).getUserCode(),"","",-1L,"","","","",1L,page,15);
		}
		if (search==true){
			this.getDataForScroll();
			listVehicles=vehicleEJB.getUserVehiclesByFilters(userEJB.getCodeTypeId(userId).longValue(),
					userEJB.getSystemUser(userId).getUserCode(),"","",categoryType, 
					placaFil, ad1Fil, ad2Fil, ad3Fil,1L,page,15);
		}
		return listVehicles;
	}
	...
	public void getDataForScroll(){
		try {
			if(search==false){
				this.setValuesFor(Integer.parseInt(String.valueOf(vehicleEJB.getUserVehiclesByFilters(userEJB.getCodeTypeId(userId).longValue(),
						userEJB.getSystemUser(userId).getUserCode(),"","",-1L,"", "", "", "",1L,0,15).get(0))));
			}else{
				this.setValuesFor(Integer.parseInt(String.valueOf(vehicleEJB.getUserVehiclesByFilters(userEJB.getCodeTypeId(userId).longValue(),
						userEJB.getSystemUser(userId).getUserCode(),"","",categoryType, placaFil, ad1Fil, ad2Fil, ad3Fil,1L,0,15).get(0))));
			}
